...
export default function MyCourseItem(props: TMyCourseItem) {
  return (
    <div className='flex gap-6 course-item'>
      ...
      <div className='flex-1 flex flex-col'>
        ...
        <div className='font-extralight text-12.5 italic'>{props.desc}</div>
      </div>
      <div className='flex flex-col gap-2 justify-center'>
        <TotalHours value={props.details.time} />
        <TotalModules value={props.details.modules} />
        <TotalVideos value={props.details.videos} />
      </div>
      <div className='flex flex-col gap-3 justify-center min-w-[225px]'>
        <span className='text-[17.5px] font-bold '>Progress: <span className='text-[#0C8D92]'>{props.percentageComplete}%</span></span>
        <CourseProgress percentage={props.percentageComplete} />
        <div className='flex gap-2'>
          {props.completed ? <><button className='btn style2'>Completed</button> <button className='btn style1'>See details</button></> : <button className='btn style1'>Resume</button>}
        </div>
...
const TotalHours = ({ value }: { value: number }) => {
  return (<SimpleBox>
    ...
    <SimpleText>Total {value} Hrs.</SimpleText>
  </SimpleBox>)
}
const TotalModules = ({ value }: { value: number }) => {
  return (<SimpleBox>
    ...
    <SimpleText>{value} Modules</SimpleText>
  </SimpleBox>)
}
const TotalVideos = ({ value }: { value: number }) => {
  return (<SimpleBox>
    ...
    <SimpleText>{value} Videos</SimpleText>
  </SimpleBox>)
}
...
const SimpleBox = ({
  ...
  children?: React.ReactNode
}) => <div className='flex gap-3 rounded-[3px] border-.5 px-2 py-1'>{children}</div>